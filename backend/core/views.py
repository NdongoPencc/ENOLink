...
from accounts.permissions import IsDFIP, IsEnseignantOrCoordinateur
from .models import (
...
class CohorteListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        qs = Cohorte.objects.select_related('specialite', 'cree_par').all()
    ...
    def perform_create(self, serializer):
        cohorte = serializer.save(cree_par=self.request.user)
        JournalAudit.log(
            self.request.user,
...
class CohorteDetailView(generics.RetrieveUpdateDestroyAPIView):
    queryset = Cohorte.objects.select_related('specialite', 'cree_par').all()
    serializer_class = CohorteSerializer
...
class RegroupementListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        qs = Regroupement.objects.select_related('cohorte', 'lance_par').prefetch_related('groupes').all()
        cohorte_id = self.request.query_params.get('cohorte_id')
        if cohorte_id:
            qs = qs.filter(cohorte_id=cohorte_id)
        return qs
...
def lancer_regroupement_view(request):
    ...
    regroupement = serializer.save(lance_par=request.user, statut=Regroupement.Statut.EN_ATTENTE)
...
def export_csv_view(request, pk):
    ...
    JournalAudit.log(
        request.user, JournalAudit.Action.EXPORT_CSV,
...
def export_pdf_view(request, pk):
    ...
    JournalAudit.log(
        request.user, JournalAudit.Action.EXPORT_PDF,
